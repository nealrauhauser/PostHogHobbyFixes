...
if [ "${TOTAL_CONTAINERS:-0}" -lt 5 ]; then
    ...
    echo "│"
    echo "│   /home/repoman/HBbackend/PostHog/setup.sh"
    echo "│"
    echo "└─────────────────────────────────────────┘"
    exit 1
fi
...
# posthog_eventschema — required by ingestion-general
...
if [ $FAIL -gt 0 ]; then
    ...
    if $HAS_SCHEMA_DRIFT; then
        ...
        echo "│ See PostHogFuckups.md Issue 11 for ALTER TABLE fixes."
    fi
    ...
    echo "│"
    echo "│ Full reference: PostHogFuckups.md"
